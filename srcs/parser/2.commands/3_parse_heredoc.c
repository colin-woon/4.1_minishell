...
/*   Updated: 2025/02/05 20:37:42 by cwoon            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_heredoc(t_cmd **last_cmd, t_token **tokens)
{
	...
	cmd->io_fds->fd_in = open(cmd->io_fds->infile, O_WRONLY | O_APPEND | O_CREAT, 0644);
...
void	run_heredoc(t_io_fds *io)
{
	...
	while (1)
	{
		...
		if (input == NULL)
			...
			free_ptr((void **)&input), free_ptr((void **)&temp));
	...
	return (free_ptr((void **)&input), free_ptr((void **)&temp));
}
